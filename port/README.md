...
## Cooperative expeditions (v0.8)

On the character screen choose **Create a party**, then share the server's network address and the eight-character code with up to three friends. Each friend chooses their own class and **Join a party**. The Party button below the left status panel opens invitations, voice controls, revival, saving and leaving. Use the host computer's LAN address from other computers; `127.0.0.1` always means the computer opening the browser. The server now listens on all interfaces; set `HOST=127.0.0.1` for local-only play. Existing solo saves remain separate.

Co-op has independent server-owned characters, inventories, equipment, health, power and progression. Movement runs at 60 Hz, with 30 Hz position updates and two full content updates per second plus immediate action updates. Enemies, damage, doors, projectiles, floor loot, opened containers and drops are shared. Pickups are atomic and commands are checked against the sending player's inventory and physical reach. Different party members can explore different loaded floors simultaneously. Empty floors pause. A disconnected character keeps its seat and inventory; use **Rejoin saved party** in the original browser. A separate secret reconnect credential is stored locally and is never included in the invitation or other players' state. Explicitly leaving frees the seat and drops that character's equipment. The leader can also release a disconnected seat from the Party menu. Rejoining with the private credential takes control from any stale connection. The leader indicator transfers on disconnect; world simulation runs on the Node server regardless of which client created the party.

Each of the thirteen classes has its own voxel outfit, headgear, weapon, walking/attacking rig and overhead name. Enemy health scales by **1 / 1.65 / 2.30 / 2.95**, and damage by **1 / 1.18 / 1.36 / 1.54**, for one through four connected adventurers. Changing party size preserves each enemy's remaining health fraction. Multiplayer has no pets. Reviving a nearby fallen ally takes **10 uninterrupted seconds**; moving, taking damage, acting, jumping, disconnecting or losing reach cancels it. A progress bar and Cancel button appear while reviving. A **full party wipe permanently ends the expedition**. Everyone must choose **New expedition** and create/join a fresh party. Ended saves retain their result and cannot be resumed into play.

The world creator owns the campaign's role, quest leader, nemesis, artifact and final altar alignment, even when party leadership transfers. Any companion can speak to that quest leader on behalf of the party. Reach level 14 as a party, accept the quest, defeat the nemesis and return with the quest artifact. Recover the Bell of Opening, Candelabrum of Invocation and Book of the Dead; gather the tools within eight metres of the vibrating square and choose **Perform invocation**. Defeat the Sanctum's high priest and recover the real Amulet. Gather all connected companions alive at the surface exit, then let the Amulet bearer lead everyone into the Elemental Planes. Reach Astral and offer it on the creator's aligned altar to **ascend together**. The Party menu shows the current objective. Native branches, ladders, portals and trapdoor destinations are connected; portals and downward drops use visible stairways in the cooperative renderer. Search reveals secret passages; **Dig** uses a carried pick-axe, mattock or digging wand on nearby rock. Earth supplies digging tools through monster drops. Shops and other native systems still have the boundaries below.

**Voice:** choose **Enable proximity voice** and grant microphone access. Hold **B** to talk, or uncheck that setting for an open microphone. Microphone mute, deafen and individual companion mute are available. Speaking names appear on the left. Volume falls to zero at 18 metres and across floors; walls muffle voices. Voice uses a maximum of three WebRTC peer links per browser, with no audio recording. Distance attenuation is applied at the receiving client, so it is an audio effect, not a confidentiality boundary.

Microphone access on another computer requires a secure browser origin. Set `NETHACK_TLS_CERT` and `NETHACK_TLS_KEY` to PEM certificate/key paths and open the resulting **HTTPS** address with a certificate trusted by all players. Alternatively, put the server behind an HTTPS reverse proxy that forwards WebSockets and preserves the Host header. Localhost works without HTTPS. For internet voice across restrictive NAT/firewalls, supply your own TURN service with `NETHACK_ICE_SERVERS`, a JSON array of browser `RTCIceServer` objects. No external ICE provider is configured by default. Internet game access also needs a reachable server address, forwarding, or a VPN. See the browser's [microphone requirements](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia) and [WebRTC configuration](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/RTCPeerConnection).

Party saves are atomic JSON files under `engine/runtime-polished-v08/parties/`, including all visited floors, enemy health/positions and private character inventories. They save every 30 seconds, on disconnect and floor changes, and via **Save party**. Saving does not disconnect the party. Keep the server runtime directory and the browser's site storage to resume your character after restarting. Separate isolated native generators live under `party-generators/`; they never use the solo character's save.

**Personal maps:** press **M** and look down to read your character's named map. Each player records only terrain they have personally seen, separately on every floor. Walls, doors, stairs and other features use the cooperative world's terrain symbols rather than the native generator's stale glyphs. The held party map updates as snapshots arrive, including your position and newly explored areas. Closed doors and walls block discovery, and changes in distant rooms remain as you last saw them until you return. Personal maps persist in party saves and reconnects; existing campaign saves upgrade each character's previous exploration automatically. Teammates do not automatically share map discoveries.

Party voice options include a **microphone device selector**, **0–200% microphone volume**, and **Test microphone** with a live input meter and local listen-back. Use headphones for the test. Testing temporarily stops transmission to companions; stopping the test restores the selected push-to-talk/mute behavior. Closing the menu also stops the test. Device selection and volume are remembered in this browser. Switching devices keeps existing voice connections. The microphone list gains device names after microphone permission is granted.

**Compatibility boundary:** co-op uses a separate cooperative ruleset over native-generated dungeon content, now including the creator's quest, native branch routing and a shared ascension path. It does not reproduce every NetHack C rule. Consumables, tools, spell effects, containers, traps, armor, combat and quest dialogue use cooperative rules; native shop billing, full artifact powers, polymorph and complex item interactions remain unsupported. The command palette lists supported actions. Party saves from before shared campaign support require a new expedition; they are preserved on disk. Solo retains its native rules. Automated tests exercise campaign state and native connectivity, but a complete manual four-person ascension playthrough and network/balance testing remain outstanding.

Run `npm run test:multiplayer` for cooperative simulation/native generation tests and a four-browser test covering player identities, remote models, all thirteen class rigs, six voice connections, received voice indication, mute, and reconnect. Screenshots are written to `test-results/party-*.png`.
...
Double-click **`Play NetHack 3D.cmd`** in the repository root. It starts the local server and opens the game in a standalone Chrome window, or your default browser if Chrome is unavailable. The polished executable is already built for Windows x64. This launcher opens version 0.8 on port 5179; the earlier builds on 5173–5178 and their save directories remain separate. Node.js 22 or newer and a browser supporting WebGL 2 are required.
...
| Z | Zap a wand |
| G | Eat |
| E | Pick up the nearby object you are facing, loot a chest/box, open a door or interact |
| L | Close the open door you are facing |
| C / T | Fire quivered ammunition / throw an item |
| I / Alt | Inventory; select an item, then Wield, Wear, Apply, Drink, Eat, Read or Drop |
...
- A continuously running server clock, independent of browser rendering and open panels. Idle time advances monsters and the rest of the world. Failed movement or free commands cannot freeze the clock. Multi-turn occupations and helplessness fade the view to black, simulate their native turns at accelerated pace, then fade back in. Hunger, damage and creatures still advance. While trapped, holding WASD attempts to escape at the native clock pace; the body stays pinned until NetHack releases it. Waiting alone does not count as an escape attempt. Menus, released/stale input, helplessness and overloading do not trigger attempts.
...
- Physical melee targeting uses generous aim assistance scaled to creature size, metre-space reach, elevation and obstruction tests, followed by native damage resolution. Each swing selects exactly one enemy, including when wielding Cleaver.
- Walkable altar platforms with four shallow stone steps. Foot support and item placement share the rendered step heights. Walk onto the platform, drop offerings from inventory, then use Tab → offer for native sacrifices.
...
`npm run test:browser` runs seven browser suites with isolated QA servers and runtimes, then closes them. It checks fractional movement, crouching, time in inventory, native saving and fractional-position restoration, and captures the game and stairwell under `test-results/`. `npm test` also checks swept collision, diagonal speed, door projection, whole-floor stair traversal in both directions and a real native level change. The polish suite also exercises true fullscreen, input repeat, overlapping movement keys, live inventory, direct floor pickup and attack acknowledgments. A separately labeled presentation fixture verifies isolated red flashes, the damage overlay, physical stair caps, fixed dust and weapon motion. The dark-fantasy presentation suite checks all four dressing families and their motion, the full longsword attack, the absence of hands/arms and gradual lighting transitions. Escape, browser pointer release, the guide, controls and exact command entry are tested in the live game. The combat suite verifies Escape in fullscreen, spell casting, held shield armor, focus-safe guard release, removal of corner icons and saving/archiving before a new run. The native tests check shield and weapon armor bonuses, encumbrance, and movement-speed ratios. Presentation fixtures cover branch palettes and build 385 source-catalog item models; this catches missing geometry and classification failures, not every possible visual detail. The immersion suite verifies native armor blackouts, physical jumping, map discovery and frozen timestamps, pickup highlighting, keyboard casting/throwing with captured aim, and Escape closing menus. Native tests also exercise eating and paralysis. The trap/altar browser suite restores a real trapped character, confirms held WASD escapes, walks up an altar, drops a corpse onto its surface and makes a successful native sacrifice. No tests use the player runtime.

`PORT`, `NETHACK_ENGINE` and `NETHACK_RUNTIME` can override the server port, executable and save/data directory. `HOST` overrides the default `0.0.0.0` listener. The server accepts same-origin HTTP/HTTPS WebSocket connections. Version 0.8 uses port 5179 and a separate runtime; earlier servers and saved expeditions can remain in place.
