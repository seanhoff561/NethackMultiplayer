...
test('native spatial anchors spend no movement turns; physical stair endpoint changes the real dungeon floor',{timeout:25000},async()=>{
  ...
  const engine=new NativeSession({executable:process.env.NETHACK_ENGINE||path.join(root,'engine/bin/nethack-engine-polished-v06.exe'),cwd,args:['-p','Wizard','-r','human','-g','male','-a','neutral','-u','StairQA']});
