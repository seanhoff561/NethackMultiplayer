...
test('native floor piles are nonmodal, individually retrievable, and emit actual pickup events',{timeout:20000},async()=>{
  ...
  const engine=new NativeSession({executable:process.env.NETHACK_ENGINE||path.join(root,'engine/bin/nethack-engine-polished-v06.exe'),cwd,args:['-p','Wizard','-r','human','-g','male','-a','neutral','-u','LootQA']});
