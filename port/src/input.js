...
export const BINDINGS = Object.freeze({
  ...
  KeyI:'inventory',AltLeft:'inventory',AltRight:'inventory',Tab:'commands',KeyE:'interact',KeyL:'closeDoor',Space:'jump',KeyM:'map',
...
export class GameInput {
  ...
  keyDown(e) {
    ...
    if(state==='menu'){
      if(['AltLeft','AltRight'].includes(e.code)){e.preventDefault();e.stopImmediatePropagation();return;}
      this.clear();
    ...
    const action=BINDINGS[e.code];if(!action||e.altKey&&!['AltLeft','AltRight'].includes(e.code)||e.metaKey)return;
    e.preventDefault();e.stopImmediatePropagation();
    if(!fresh||e.repeat)return;
    if(continuous.has(action))this.active.set(e.code,action);
    else this.onAction(action);
  }
  keyUp(e){if(['AltLeft','AltRight'].includes(e.code))e.preventDefault?.();this.held.delete(e.code);this.active.delete(e.code);}
