...
export class PartyUI {
  constructor({send,connect,onNotice,onMenu,onRevive,onLeave}){
    Object.assign(this,{send,connect,onNotice,onMenu,onRevive,onLeave});this.players=[];this.voice=new PartyVoice({send,onStatus:text=>{this.voiceStatus.textContent=text;}});
    ...
    button('#voice-enable',async()=>{const b=this.panel.querySelector('#voice-enable');b.disabled=true;try{if(this.voice.enabled){this.voice.disable();b.textContent='Enable proximity voice';this.voiceStatus.textContent='Microphone is off.';}else {await this.voice.enable();b.textContent='Disable voice';}this.panel.querySelector('#voice-mute').disabled=!this.voice.enabled;this.panel.querySelector('#voice-deafen').disabled=!this.voice.enabled;}catch(e){this.voiceStatus.textContent=e.message;}finally{b.disabled=false;}});
  ...
  handle(data){
    if(data.type==='party-result'){this.result=data;this.panel.hidden=true;this.reviveHud.hidden=true;this.resultPanel.hidden=false;this.resultPanel.querySelector('h1').textContent=data.status==='victory'?'You ascend together.':'The party has fallen.';this.resultPanel.querySelector('p').textContent=data.text;return true;}
