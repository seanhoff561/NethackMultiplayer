...
if (-not (Test-Path -LiteralPath (Join-Path $gameRoot 'engine/bin/nethack-engine-polished.exe'))) {
    & (Join-Path $gameRoot 'engine/build.ps1')
}
if (-not (Test-Path -LiteralPath (Join-Path $gameRoot 'dist-polished/index.html'))) {
    Push-Location $gameRoot
...
if (-not $running) {
    ...
    Start-Process -FilePath $nodePath -ArgumentList 'server.mjs' -WorkingDirectory $gameRoot -WindowStyle Hidden -RedirectStandardOutput (Join-Path $gameRoot '.polished-server.log') -RedirectStandardError (Join-Path $gameRoot '.polished-server-errors.log')
    for ($attempt = 0; $attempt -lt 30; $attempt++) {
        Start-Sleep -Milliseconds 200
        try { $status = Invoke-RestMethod "$gameUrl/api/status" -TimeoutSec 1; if ($status.ready) { $running = $true; break } } catch {}
    }
}
if (-not $running) { throw 'The game server did not start. See port/.polished-server-errors.log.' }
