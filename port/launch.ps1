...
if (-not (Test-Path -LiteralPath (Join-Path $gameRoot 'engine/bin/nethack-engine-spatial.exe'))) {
    & (Join-Path $gameRoot 'engine/build.ps1')
}
if (-not (Test-Path -LiteralPath (Join-Path $gameRoot 'dist-spatial/index.html'))) {
    Push-Location $gameRoot
...
if (-not $running) {
    ...
    Start-Process -FilePath $nodePath -ArgumentList 'server.mjs' -WorkingDirectory $gameRoot -WindowStyle Hidden -RedirectStandardOutput (Join-Path $gameRoot '.spatial-server.log') -RedirectStandardError (Join-Path $gameRoot '.spatial-server-errors.log')
    for ($attempt = 0; $attempt -lt 30; $attempt++) {
        Start-Sleep -Milliseconds 200
        try { $status = Invoke-RestMethod "$gameUrl/api/status" -TimeoutSec 1; if ($status.ready) { $running = $true; break } } catch {}
    }
}
if (-not $running) { throw 'The game server did not start. See port/.spatial-server-errors.log.' }
