...
export class PartyDungeon {
  ...
  async start(){
    ...
    const e=this.engine=new NativeSession({executable:this.executable,cwd:this.cwd,args:['-D','-p',c.role,'-r',c.race,'-g',c.gender,'-a',c.alignment,'-u','wizard'],env:{NH_COOP_GENERATOR:'1'}});
  ...
  async until(predicate){const deadline=Date.now()+12000;while(!predicate()){if(this.error||this.engine.closed||Date.now()>deadline)throw Error(this.error||'Dungeon generation timed out.');await new Promise(resolve=>setTimeout(resolve,15));}}
  floor(depth){
    const task=this.queue.then(async()=>{
      await this.start();const e=this.engine;
      if(typeof depth==='string'){
        if(!/^\d+:\d+$/.test(depth))throw Error('Invalid native floor identifier.');
        if(e.snapshot.levelId!==depth){e.write({kind:'campaign-floor',value:depth.split(':').map(Number)});await this.until(()=>e.snapshot.levelId===depth);}
