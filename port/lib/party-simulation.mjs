...
export class PartySimulation {
  ...
  snapshot(p){
    ...
    this.survey(p);p.mapRevisionSent=p.cartography?.[p.depth]?.revision||0;
  ...
  motion(p){
    const f=this.floors.get(p.depth);this.survey(p);const map=this.personalMap(p),cartography=p.mapRevisionSent===map.revision?undefined:map;p.mapRevisionSent=map.revision;
    return {type:'motion',...(cartography?{cartography}:{}),campaign:this.campaign?.packet(),revive:this.reviveState(p),levelId:`party:${p.depth}`,time:this.time,player:p.body,blocked:!!this.outcome||p.hp<=0||!!p.transition,transition:!!p.transition,players:[...this.players.values()].map(x=>this.publicPlayer(x)),actors:f?[...f.sim.actors.values()].map(a=>({id:a.id,x:a.x,z:a.z,y:a.y,yaw:a.yaw,moving:!!a.moving,visible:true})):[],projectiles:f?.projectiles.map(({id,x,y,z,kind})=>({id,x,y,z,kind}))||[]};
  }
