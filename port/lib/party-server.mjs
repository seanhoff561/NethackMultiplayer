...
export class PartyServer {
  constructor({root,runtime,executable,iceServers=[]}){Object.assign(this,{root,runtime,executable,iceServers});this.rooms=new Map();this.connections=new Map();this.tickCount=0;this.roomIdleMs=60_000;}
  ...
  cleanupInactive(force=false){
    const now=Date.now();
    let removed=false;
    for(const [id,room] of [...this.rooms]){
      const active=room.loading?.size||[...room.players.values()].some(p=>p.connected);
      if(active){room.inactiveSince=null;continue;}
      room.inactiveSince??=now;
      if(!force&&now-room.inactiveSince<this.roomIdleMs)continue;
      if(!this.save(room))continue;
      room.generator?.stop?.();this.rooms.delete(id);removed=true;
  ...
  room(code,create){
    ...
    if(this.rooms.size>=8)this.cleanupInactive(true);
  ...
  disconnect(ws){if(ws.party){const {room,id}=ws.party;this.connections.delete(id);room.disconnect(id);if(![...room.players.values()].some(p=>p.connected))room.inactiveSince=Date.now();ws.party=null;}}
  tick(dt){
    this.tickCount++;
    ...
    if(this.tickCount%120===0)this.cleanupInactive(false);
  }
